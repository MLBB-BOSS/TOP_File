...
async def send_character_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Можна реалізувати показ загальної інформації або запросити вибір конкретного героя
    buttons = []
    heroes = heroes_data.get('heroes', [])
    for hero in heroes:
        buttons.append([KeyboardButton(hero['name'])])
        if len(buttons[-1]) == 4:
            continue
    buttons.append([KeyboardButton("🔙 Назад")])
    reply_markup = ReplyKeyboardMarkup(buttons, resize_keyboard=True, one_time_keyboard=False)
    await update.message.reply_text("📝 **Оберіть героя для перегляду деталей:**", parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup)
...
# Приклад функцій для інших підменю можна реалізувати аналогічно
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            States.CHARACTERS_MENU: [
                MessageHandler(filters.Regex(f"^({'|'.join([hero['name'] for hero in heroes_data.get('heroes', [])])})$"), handle_hero_selection),
            ...
            # Додай інші стани тут за потребою
        },
