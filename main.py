...
class States(Enum):
    ...
    TANK_CHARACTERS_MENU = auto()  # Новий стан для підменю Танків

# Налаштування OpenAI API
OPENAI_API_KEY = os.getenv('OPENAI_API_KEY')
API_URL = "https://api.openai.com/v1/chat/completions"

# Визначення тригерних слів та фраз
TRIGGER_WORDS = ["бот", "GPT", "допоможи", "розкажи"]

# Додавання шаблонів відповідей для особистості
...
# Список персонажів класу "Танк"
TANK_CHARACTERS = [
    "Танк 1", "Танк 2", "Танк 3", "Танк 4", "Танк 5",
    "Танк 6", "Танк 7", "Танк 8", "Танк 9", "Танк 10",
    "Танк 11", "Танк 12", "Танк 13", "Танк 14", "Танк 15",
    "Танк 16", "Танк 17", "Танк 18"
]

# Змінна для відстеження останнього повідомлення
last_message_time = {}
...
# Функція для оновлення часу останнього повідомлення
...
async def trigger_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if any(trigger in message_text for trigger in TRIGGER_WORDS):
        ...
        await handle_gpt_query(update, context, update.message.text)
...
async def check_bot_permissions(context: ContextTypes.DEFAULT_TYPE):
    try:
        bot = context.bot  # Отримання об'єкту бота
...
# Функція для автоматичної розсилки після паузи
async def automatic_broadcast(context: ContextTypes.DEFAULT_TYPE):
    await asyncio.sleep(120)  # Початкова затримка 2 хвилини
    while True:
        current_time = datetime.utcnow()
        for user_id, last_time in last_message_time.items():
            if current_time - last_time > timedelta(minutes=2):
                # Відправка повідомлення
                await send_message(context, user_id, "Чи потрібна вам допомога? Я завжди готовий допомогти!")
...
async def send_message(context: ContextTypes.DEFAULT_TYPE, chat_id: int, message: str):
...
# Обробник повідомлень для основного меню та підменю
...
async def handle_characters_class_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if selected_class in classes:
        ...
        if heroes:
            if selected_class == "Танк":
                # Перехід до підменю Танків з 18 персонажами
                buttons = []
                for i in range(0, len(TANK_CHARACTERS), 3):
                    row = TANK_CHARACTERS[i:i + 3]
                    buttons.append([KeyboardButton(hero) for hero in row])
                buttons.append([KeyboardButton("🔙 Назад")])
                reply_markup = ReplyKeyboardMarkup(buttons, resize_keyboard=True, one_time_keyboard=False)
                await update.message.reply_text(f"🗂 **Список танків:**", parse_mode='Markdown', reply_markup=reply_markup)
                return States.TANK_CHARACTERS_MENU
            else:
                # Для інших класів можна реалізувати аналогічно або іншим чином
                buttons = []
        ...
        await handle_characters_menu(update, context, "🗂 Список героїв")
        return States.CHARACTERS_MENU
    else:
...
# Функції для обробки підменю Танків з 18 персонажами
async def handle_tank_characters_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    selected_hero = update.message.text.strip()
    if selected_hero in TANK_CHARACTERS:
        hero_info = get_hero_info(selected_hero)
        if hero_info:
            hero_details = (
                f"📖 **Деталі про танка {selected_hero}:**\n\n"
                f"<b>Клас:</b> {hero_info.get('class', 'Невідомо')}\n"
                f"<b>Роль:</b> {hero_info.get('role', 'Невідомо')}\n"
                f"<b>HP:</b> {hero_info.get('hp', 'Невідомо')}\n"
                f"<b>Атака:</b> {hero_info.get('attack', 'Невідомо')}\n"
            ...
            buttons.append([KeyboardButton("🔙 Назад")])
            reply_markup = ReplyKeyboardMarkup(buttons, resize_keyboard=True, one_time_keyboard=False)
            await update.message.reply_text("🗂 **Список танків:**", parse_mode='Markdown', reply_markup=reply_markup)
            return States.TANK_CHARACTERS_MENU
        else:
            await update.message.reply_text("⚠️ Не вдалося знайти інформацію про цього танка.")
            return States.TANK_CHARACTERS_MENU
    elif selected_hero == "🔙 Назад":
        await select_hero_class(update, context)
        return States.CHARACTERS_MENU
    else:
        await update.message.reply_text("⚠️ Вибрана опція не відповідає жодному танку.")
        await select_hero_class(update, context)
        return States.CHARACTERS_MENU
...
# Функція для старту вікторини "Вгадай цитату"
...
async def handle_search_performing(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query:
        ...
        if matching_heroes:
            if "Танк" in query or "танк" in query:
                # Перехід до підменю Танків
                buttons = []
                for i in range(0, len(TANK_CHARACTERS), 3):
                    row = TANK_CHARACTERS[i:i + 3]
                    buttons.append([KeyboardButton(hero) for hero in row])
...
# Функція для перевірки паузи та відправки повідомлень
async def check_inactivity(context: ContextTypes.DEFAULT_TYPE):
    await automatic_broadcast(context)
...
async def main():
    ...
    # Додаємо обробник повідомлень з тригерними словами
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, message_handler))
    
    # Додаємо ConversationHandler для більш складних сценаріїв (підменю, вікторини тощо)
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            States.CHARACTERS_MENU: [
                ...
                MessageHandler(filters.Regex("^Танк \d+$"), handle_tank_characters_menu)  # Обробка Танків
            ],
            ...
            States.QUIZZES_MENU: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_quizzes_menu),
                MessageHandler(filters.Regex("^Танк \d+$"), handle_tank_characters_menu)  # Обробка Танків
            ],
            States.TANK_CHARACTERS_MENU: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_tank_characters_menu)],
    ...
    # Запускаємо асинхронну задачу для перевірки паузи
